...
- Five checkpoint routes with saved personal bests: the **Meridian Loop** (pure top speed), the **Riverline Sprint** (speed-gated water running), **The Crest Ladder** (rooftop-only, wall running required), the **Kestrel Courier** chain, and the **Wake Chain** (unbroken slipstream — brake and the gate ignores you). Route PBs also store a local replay ghost.
- Two rescue runs — reach every stranded person before the clock runs out; each save buys you time back. A **speed vortex** (orbit hard at pace) or a **remnant echo** can clear people you cannot reach alone.
...
- Six opt-in **rogue duels** parked at landmarks, including **Anchor** — a boss that plants dead-zones which strip wall runs, water runs and top speed instead of merely growing a health bar. Combat is a minigame you choose to start, not something that chases you across the map.
...
| **Solid phase** | V — vibrate through matter for a beat; costs momentum, grants brief invulnerability |
...
| **Remnant echo** | G — spawn a short-lived copy that replays your recent path (decoy, second rescue body) |
| **Speed vortex** | Sustain a hard high-speed orbit — pulls nearby rescues and light foes inward |
| **Mass strike** | Left click at high pace — knockback and damage scale with momentum |
| Focus time | Hold F — everyone else slows, you don't; drains momentum; marks the nearest threat |
...
| Ctrl / C | Slide |
| Mouse | Look |
| Left click | Speed strike · mass strike at pace |
| E | Arc bolt |
| Q | Kinetic pulse |
| V | Solid phase |
| G | Remnant echo |
...
Gamepad is supported: sticks to move and look, A jump, B slide, X strike, Y bolt, bumpers for focus and pulse, extra face buttons for phase and remnant.
...
src/
  game/
    ...
    player/      Player (traversal state machine), HeroModel, Remnant (echo)
    ...
    activities/  Activity contract, RouteRun, RouteGhost, RescueRun, RogueDuel, Collectibles, routes
...
### Resonance Mastery (this expansion)

- [x] Solid phase — vibrate through buildings briefly
- [x] Momentum mass strike — damage and knockback scale with pace
- [x] Remnant echo — path-replaying decoy / second rescue body
- [x] Speed vortex — orbit-built pull for rescues and light foes
- [x] Anchor — free-roam boss that denies wall, water and top speed
- [x] Focus threat marking
- [x] Local route PB ghosts + Wake Chain slipstream route
...
- Focus time that slows selected simulation layers rather than a single multiplier, with route planning.
- Audio: footsteps, wind, cloth, impacts, electricity, ambience, dialogue and adaptive music.
- More bosses that change traversal rules (beyond Anchor).
- Asynchronous online leaderboards (local ghosts ship now).
