...
The speedster is the focal point. Nolan's suit combines garnet ceramic surfaces, graphite fabric joints, ivory shoulder/knee plates and restrained champagne-metal trim. A closed helmet and curved visor create a continuous head silhouette. Split amber chevrons across the scapulae continue down the spine; wrist/calf/heel inlays keep the character recognisable from behind. Focus changes the energy colour to blue.

The character is constructed from explicit elliptical cross-sections, interpolated into capped meshes with computed normals. Separate pieces sit on the existing named joint hierarchy. This preserves the traversal controller while replacing its visible body. Actual animated descendants replace the upright capsule shadow proxy. Idle stops cycling the limbs; running cadence, banking and the transitions into airborne, slide and wall poses are blended.

The menu uses this same rig in a live portrait, and gameplay uses a closer chase camera. The portrait is obscured by a solid menu background on narrow displays to prioritise controls and legibility.
...
| --- | --- |
| Main character | 29,104 vertices, 72 meshes including the empty shadow root |
...
| City | 723 meshes including the test hero; 208 structure shadow casters in the High test fixture |
...
Local browser preview access was rejected by the browser permission check during this change. No screenshots, browser play-test results, GPU compatibility results or FPS claims are supplied.
